...
kable(addmargins(M), caption = "Reakcja na lek A (wiersze) vs lek B (kolumny).",
      booktabs = TRUE) |>
...
tab_z2 <- data.frame(
  ...
  Statystyka   = c(round(unname(mcc$statistic), 4), NA),
...
tab_z3 <- data.frame(
  Test        = c("McNemar (z poprawką na ciągłość)", "warunkowy (zad. 1)"),
  Statystyka  = c(round(unname(mcc3$statistic), 4), NA),
...
Dane z Tabeli 2 (pytanie 1 vs pytanie 2 w obu okresach):
...
kable(T2, caption = "Tablica 5x5 odpowiedzi w dwóch okresach (Tabela 2).",
      booktabs = TRUE) |>
  kable_styling(latex_options = c("HOLD_position"))
```
...
k <- nrow(T2); bow <- 0; df_eff <- 0; zero_pairs <- c()
for (i in 1:(k-1)) for (j in (i+1):k) {
  s <- T2[i,j] + T2[j,i]
  if (s > 0) { bow <- bow + (T2[i,j]-T2[j,i])^2 / s; df_eff <- df_eff + 1 }
  else zero_pairs <- c(zero_pairs, paste0("(",i,",",j,")"))
}
df_nom <- k*(k-1)/2
tab_z4 <- data.frame(
...
kable(tab_z4, caption = "Test Bowkera symetrii dla tablicy 5x5.",
      booktabs = TRUE) |>
  kable_styling(latex_options = c("HOLD_position"))
```
...
**Wniosek.** Brak podstaw do odrzucenia hipotezy symetrii — odpowiedzi w
obu okresach odpowiadają modelowi symetrii. Wobec tego **nie ma dowodu na
zmianę oceny podejścia firmy**: ocena w pierwszym i drugim okresie jest
statystycznie nieodróżnialna pod względem symetrii rozkładu.
...
tab5 <- data.frame(
  ...
  `Poprawa A` = c(117, 17, 100),
  `Poprawa B` = c(177, 2, 175),
...
kable(tab5, caption = "Odsetek poprawy i iloraz szans dla leczenia A vs B.",
      booktabs = TRUE) |>
  kable_styling(latex_options = c("HOLD_position"))
```
...
u trudniejszych pacjentów (z chorobami współistniejącymi: 118 vs 38 osób),
co pogorszyło jej zagregowany wynik. Właściwą oceną skuteczności jest analiza
...
**Opis Rysunku 4.** Wykres przedstawia rozkład odpowiedzi PYT\_2 w sześciu
